...
branch. That keeps `agentfix --version` from importing LangGraph or the eval harness, and it
...
from agentfix.agent.graph import MAX_STEPS
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "solve":
        ...
        try:
            ...
            print("Run `agentfix doctor` to check the setup.", file=sys.stderr)
            return 1
        ...
        # Exit code follows the verdict, so `agentfix solve ... && echo ok` behaves sensibly and
        # CI can gate on it.
