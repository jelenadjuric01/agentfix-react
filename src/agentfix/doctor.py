...
from agentfix.config import BASE_MODEL, FALLBACK_MODEL, MIN_CONTEXT_LENGTH, LLMConfig

PULL_HINT = f"run: ollama pull {BASE_MODEL}"
CREATE_HINT = "run: ollama create agentfix-mellum2-thinking -f Modelfile"
SERVE_HINT = (
...
def _check_reasoning_and_tools(config: LLMConfig) -> list[Check]:
    ...
    if thinking:
        ...
        reasoning_check = Check(
            ...
            "`reasoning=True` is not reaching the server; see src/agentfix/llm/client.py",
        )
    else:
        reasoning_check = Check(
            "reasoning",
